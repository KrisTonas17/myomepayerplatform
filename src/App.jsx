...
function WhatWeScreen() {
  const analysisTypes = [
    {
      ...
      color: C.blue,
      tagline: 'Genome-wide population risk — applies to every member',
      body: 'Aggregates hundreds of thousands of common genetic variants across the entire genome to calculate each person\'s population-level risk profile. This isn\'t about finding a single rare gene — it\'s about quantifying where someone sits on the risk curve across the whole population. A member in the top 10% for cardiovascular disease risk looks "normal" in a standard checkup. PRS finds them before the event. This analysis scales to your entire member population, not just rare variant carriers.',
    },
    {
      icon: '🧬',
      title: 'Single-Gene (Monogenic) Risk',
      color: C.teal,
      tagline: '151 curated high-penetrance genes',
      body: 'Looks for rare but high-impact variants in 151 clinically validated genes — things like BRCA1/2 for hereditary cancer, or inherited cardiac arrhythmia genes. These findings are less common in the population (roughly 1 in 50–100 members), but when found, the clinical and financial consequences of missing them are severe. The 151-gene panel is the "what we report" layer — but it is built on top of whole-genome data, meaning as science evolves, new findings can be requeried from the same underlying data.',
    },
    {
      icon: '⚗️',
      title: 'Pharmacogenomics (PGx)',
      color: C.amber,
      tagline: 'Medication response mapping — act before the first prescription',
      body: 'Identifies specific gene variants that control how each person\'s body processes medications. Determines whether a member is a fast, slow, or normal metabolizer for specific drug classes — and which medications carry elevated risk for adverse reactions. This information enables better medication decisions before the first dose is written, reducing failed therapies, prescription switching, adverse drug events, and the downstream utilization they drive.',
    },
  ]
  ...
  const categories = [
    ...
    },
    {
      ...
      color: C.amber,
      conditions: [
        'How a member metabolizes certain medications (fast, slow, or normal)',
        'Risk of adverse reactions to specific drug classes',
        'Antidepressant and psychiatric medication response',
        'Cardiovascular medication metabolism',
        'Pain medication response and tolerance',
        'Chemotherapy tolerance and response factors',
      ],
      whyItMatters: 'Adverse drug events are among the most preventable high-cost healthcare events. Knowing how a member\'s genes affect medication processing can guide prescribing before the first dose — reducing failed therapies, hospitalizations, and switching costs.',
    },
    {
      icon: '🧬',
      ...
      <div style={{ ...sectionWrap, position: 'relative' }}>
        <div style={{ textAlign: 'center', marginBottom: 56 }}>
          ...
          <h2 style={{
            fontFamily: fontDisplay, fontSize: 'clamp(28px, 4vw, 44px)',
            fontWeight: 800, letterSpacing: '-0.02em', color: C.textPrimary, lineHeight: 1.1,
          }}>One genome.<br />Three types of analysis.</h2>
          <p style={{ color: C.textSecondary, fontSize: 17, maxWidth: 620, margin: '16px auto 0', fontWeight: 300, lineHeight: 1.65 }}>
            MyOme sequences the entire genome — all three billion base pairs — using whole-genome sequencing (WGS). From that single data asset, three distinct analyses are run. Each one produces a different category of clinical and financial value. Together, they create a risk intelligence layer that no claims database or lab panel can replicate.
          </p>
        </div>
          ...
          {analysisTypes.map((a, i) => (
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 16 }}>
                ...
                <div>
                  <div style={{ fontFamily: fontDisplay, fontSize: 15, fontWeight: 700, color: C.textPrimary, lineHeight: 1.2 }}>{a.title}</div>
                  <div style={{ fontSize: 11, color: a.color, fontWeight: 600, marginTop: 3, letterSpacing: '0.04em' }}>{a.tagline}</div>
                </div>
              </div>
              <p style={{ color: C.textSecondary, fontSize: 14, lineHeight: 1.75, fontWeight: 300 }}>{a.body}</p>
            </div>
          ))}
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 32 }}>
          ...
          <span style={{ color: C.textMuted, fontSize: 12, fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', fontFamily: fontDisplay, whiteSpace: 'nowrap' }}>
            Condition categories covered by the 151-gene panel
          </span>
          <div style={{ flex: 1, height: 1, background: C.borderSubtle }} />
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 8, marginBottom: 32 }}>
          {categories.map((cat, i) => (
            ...
            >
              ...
              <div style={{ fontFamily: fontDisplay, fontSize: 14, fontWeight: 700, color: open === i ? cat.color : C.textPrimary, lineHeight: 1.3 }}>{cat.title}</div>
            </button>
          ))}
        </div>
        ...
        <div style={{
          ...
          <div style={{ padding: '32px', borderBottom: `1px solid ${C.borderSubtle}`, display: 'flex', alignItems: 'center', gap: 16 }}>
            <div style={{
            ...
            <div>
              ...
              <p style={{ color: categories[open].color, fontSize: 13, fontWeight: 600, marginTop: 2 }}>Conditions covered in this category</p>
            </div>
          </div>
